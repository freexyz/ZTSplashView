 ...
 * DbgMsg.h -
 *
 * Copyright (c) 2016-2018 by ZealTek Electronic Co., Ltd.
 *
 ...
 *
 * History:
 *	2016.12.24	T.C. Chiu	frist edition
 *
 ******************************************************************************
 */
 ...
 */
    ...
    #define LOG_FORMAT_NO_LOCATION(fmt, lvl, ...)   \
        NSLog((@"[%@][%@] " fmt), lvl, [[NSThread currentThread] isMainThread] ? @"Y" : @"N", ##__VA_ARGS__)

    #define LOG_FORMAT_WITH_LOCATION(fmt, lvl, ...) \
        NSLog((@"%s[Line %d] [%@][%@] " fmt), __PRETTY_FUNCTION__, __LINE__, lvl, [[NSThread currentThread] isMainThread] ? @"Y" : @"N", ##__VA_ARGS__)
#else
    #define LOG_FORMAT_NO_LOCATION(fmt, lvl, ...)   \
...
// Debug logging - use only temporarily for highlighting and tracking down problems
	...
	#define LogDbg(fmt, ...)            LOG_FORMAT(fmt, @"DEBUG", ##__VA_ARGS__)
#else
	#define LogDbg(...)
#endif
